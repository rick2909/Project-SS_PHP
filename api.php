...
function getRooster($arguments, $mysqli){
    ...
    if(strlen($id) > 0){
        ...
        if (mysqli_num_rows($result) == 1){
            ...
            if( isset($roosterId) ){
                $query = "SELECT * FROM `client_in_rooster` WHERE `rooster_id` = '$roosterId' " . ($isDagGegeven ? "AND `dag_van_de_week` = $opgrvraagdeDag ORDER BY ": "ORDER BY `dag_van_de_week` ASC, ") . "`tijd` ASC";
